...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(nonnull NSArray<CLLocation *> *)locations{
    ...
    if (location) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            [[NSUserDefaults standardUserDefaults] setFloat:location.coordinate.latitude forKey:kCurrentLatitude];
            [[NSUserDefaults standardUserDefaults] setFloat:location.coordinate.longitude forKey:kCurrentLongitude];
        });
        ...
        [[CLGeocoder new] reverseGeocodeLocation:location completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
            ...
            //得到实际的用户位置
            if ([localCity isEqualToString:kCurrentRealityCity]) {
                [[NSUserDefaults standardUserDefaults] setObject:localCity forKey:kCurrentRealityCityName];
            }
            
            //排除掉非联网状态时, 无法反地理编码得到城市名称问题.
            if (![kCurrentCity isEqualToString:localCity] && localCity!= nil) {
                //BlocksKit第三方的优化
                NSString *message = [NSString stringWithFormat:@"当前城市发生变化,是否要切换到'%@市'?", localCity];
